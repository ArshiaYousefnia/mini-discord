...
from chat.models import (
    Conversation, Channel, ConversationMember, Role, ChannelMessage
)
...
class MessageDeletionPermissionTests(APITestCase):
    def setUp(self):
        ...
        self.moderator = User.objects.create_user(
            ...
            password='pass', display_name='Mod'
        )
        self.normal_user = User.objects.create_user(
            username='normal', email='normal@test.com',
            password='pass', display_name='Normal'
        )
        self.sender = User.objects.create_user(
            username='sender', email='sender@test.com',
            password='pass', display_name='Sender'
        )
        ...
        self.channel = Channel.objects.create(
            conversation=self.conversation, is_private=False
        )

        self.mod_role = Role.objects.create(
            conversation=self.conversation, name="Moderator",
            can_delete_messages=True
        )
        self.normal_role = Role.objects.create(
            conversation=self.conversation, name="Member",
    ...
    def create_message(self):
        # Must create a ChannelMessage, not a plain Message,
        # otherwise the queryset won't find it.
